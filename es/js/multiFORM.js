...
var validateFormNorm = function(user1, user2){
	...
	if (cats1.length == 0 && cat1 == 'Al'){
		errorWriter(users[user1]+' no tiene ningún tema definido en su perfil!', 'Por favor seleccione un tema.');
		status = false;
	}
	else if (cats2.length == 0 && cat2 == 'Al'){
		errorWriter(users[user2]+' no tiene ningún tema definido en su perfil !', 'Por favor seleccione un tema.');
		status = false;
	}
...
var validateFormSel = function(user1, user2){
	...
	if(mulData[0]=='' || mulData[1]=='' || mulData[2]=='' || mulData[3]==''){
		...
		errorWriter("Formar poco lleno!", "Por favor, complete todos los campos marcados con: '*'.");
		status = false;
	}
	else if(/[^a-zA-Z ]/.test(Pal1)){
		errorWriter(users[user2]+' no puede utilizar caracteres especiales!', 'Por favor, escriba una palabra nueva.');
		status = false;
	}
	else if(/[^a-zA-Z ]/.test(Pal2)){
		errorWriter(users[user1]+' no puede utilizar caracteres especiales!', 'Por favor, escriba una palabra nueva.');
		status = false;
	}
	else{
...
var validateForm = function(){
	...
	if(user1 != user2){
		...
		errorWriter('Los Nombres de Usuario son los mismos!', 'Por favor, seleccione dos usuarios diferentes.');
		status = false;
	}
	return status
};
