...
var letter_random = function(w, U){
	...
	if (chosenLetters[U-1].indexOf(letterAl) < 0){
		var answer = confirm("La letra elegida al azar es: "+letterAl+"\nAceptar?");
		if (answer){
...
var endConst = function(U){
	...
	if (win[0]){
		$('#endGameTilte1').html('Ganó! :)');
	}
	else{
		$('#endGameTilte1').html('Perdió! :(');
	}
	if (win[1]){
		$('#endGameTilte2').html('Ganó! :)');
	}
	else{
		$('#endGameTilte2').html('Perdió! :(');
	}
