...
var mudaTabela = function () {
    ...
    if (button.html() === "Ver por Jogador") {
        button.html("Ver por Jogo");
        $("#tabelaJogo").css('display', 'none');
        $("#tabelaJogador").css('display', 'table');
    } else {
        button.html("Ver por Jogador");
...
function main () {
	var users = getUsers();
		...
		if (points.length == 0){
			errorWriter("Ainda nao houveram jogos!", "Pode jogar <a href='play.html'>aqui</a>");
		}
		else{
