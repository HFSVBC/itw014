...
var validateFormNorm = function(user1, user2){
	...
	if (cats1.length == 0 && cat1 == 'Al'){
		errorWriter(users[user1]+' não tem nenhum tema definido no seu perfil!', 'Por favor selecione um tema.');
		status = false;
	}
	else if (cats2.length == 0 && cat2 == 'Al'){
		errorWriter(users[user2]+' não tem nenhum tema definido no seu perfil!', 'Por favor selecione um tema.');
		status = false;
	}
...
var validateFormSel = function(user1, user2){
	...
	if(mulData[0]=='' || mulData[1]=='' || mulData[2]=='' || mulData[3]==''){
		...
		errorWriter(users[user2]+' não pode usar caracteres especiais!', 'Por favor escreva uma nova palavra.');
		status = false;
	}
	else if(/[^a-zA-Z ]/.test(Pal2)){
		errorWriter(users[user1]+' não pode usar caracteres especiais!', 'Por favor escreva uma nova palavra.');
		status = false;
	}
	else{
...
var validateForm = function(){
	...
	if(user1 != user2){
		...
		errorWriter('Nomes de utilizador iguais!', 'Por favor selecione dois utilizadores diferentes.');
		status = false;
	}
	return status
};
